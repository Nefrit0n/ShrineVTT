...
export const createScenesRouter = ({ sceneUseCases }) => {
  if (!sceneUseCases) {
    throw new Error("sceneUseCases dependency is required");
  }
  ...
  router.post("/", requireAuth, requireRole(RolesEnum.MASTER), async (req, res) => {
    ...
    try {
      const scene = await sceneUseCases.createScene({
        roomId,
        name: data.name,
  ...
  router.get("/:sceneId", requireAuth, async (req, res) => {
    ...
    try {
      const { scene, tokens } = await sceneUseCases.getSceneWithTokens(
        req.params.sceneId,
        pagination
      );
  ...
  router.patch(
    ...
    async (req, res) => {
      ...
      try {
        const existing = await sceneUseCases.getScene(req.params.sceneId);

        if (existing.roomId !== roomId) {
          return res.status(403).json({ error: "Access to this scene is forbidden" });
        }

        const updated = await sceneUseCases.updateScene(existing.id, data);
...
export default createScenesRouter;
